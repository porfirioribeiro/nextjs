...
function Jash() {
	...
	this.domGetElFunctions = {
		...
		className: ["getElementsByClassName","$C"]
	};
	...
	*/
	this.main = function() {
		...
		this.mainBlock=null;						/* HTML element parent wrapper */
		this.output = document.getElementById("JashOutput");	/* HTML element for console output */
		this.input=null;						/* HTML element for user input */
		this.outputHistory = [];			/* All output is stored here */
		...
		this.defaultText += line + "\n" + this.tips[(parseInt((Math.random()*10)%this.tips.length,0))] + "\n" + line + "\n";
		/**
		* import stylesheet
		*/
	...
	*/
	this.getDimensions = function(el) {
		...
		if(document.all) {
			...
			dims.x = parseInt(document.defaultView.getComputedStyle(el,"").getPropertyValue("width"),0);
			dims.y = parseInt(document.defaultView.getComputedStyle(el,"").getPropertyValue("height"),0);
		}
		return dims;
	};
	/**
...
Jash.Indenter = {
	indentChar: "\t",
	nodesCommonlyUnclosed: ["link ", "img ", "meta ", "!DOCTYPE ", "input ", "param", "hr", "br"],
	/**
	...
	*/
	this.loop = function(kind,reps) {
		...
		if(!this.results[kind][repsMemberName]) {
			this.results[kind][repsMemberName] = [];
		}
...
Jash.TabComplete = function() {
	...
	**/
	this.searchInputForDomGetElFunctions = function(inputText) {
		...
		for(i = 0; i<this.domGetElFunctions.className.length; i++) {
